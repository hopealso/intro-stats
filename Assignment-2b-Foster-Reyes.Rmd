...
```{r 9-q24}
mf.mean.diff <- mean(am.df$Anger.Out[am.df$Gender == 1]) - mean(am.df$Anger.Out[am.df$Gender == 2])
```

Difference of means between male Anger-Out and female Anger-Out scores in sample: 
$M_{AOm}$ - $M_{AOf}$ = `r mf.mean.diff`
...
Population variance for male Angler-Out scores: $\sigma_{AOm}^2$ = 20

Population variance for female Angler-Out scores: $\sigma_{AOf}^2$ = 20

### a.

Mean of the sampling distribution of the difference between means:
$\mu_{M_{AOm}}$ - $\mu_{M_{AOf}}$ = $\mu_{AOm}$ - $\mu_{AOf}$ = 2
