...
$SSE = \sum_{i}^{n} (x_{Aom_{i}} - \bar{x}_{CIa})^2 + \sum_{i}^{n} (x_{AOf_i} - \bar{x}_{CIn})^2$
...
Our goal was to determine if there is a difference between the extent to which athletes and non-athletes control angry feelings by calming down or cooling off.
...
aiao.sample.mean.diff <- mean(aiao.df$AIAO.diff)
```
...
$\bar{x}_{AI-AO}$ = `r aiao.sample.mean.diff`
...
Because we do not know the variance of the values in the population, we will use the variance within our sample to estimate the standard error in the sampling distribution.
